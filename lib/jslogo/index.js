...
(function () {
    ...
    function resize() {
        var box = $('#display-panel .inner'), rect = box.getBoundingClientRect(), w = rect.width, h = rect.height;
        $('#sandbox').width = w;
        $('#sandbox').height = h;
        $('#turtle').width = w;
        $('#turtle').height = h;
...
//
(function () {
    var sidebars = Array.from($$('#sidebar .choice')).map(function (elem) { return elem.id; });
...
window.addEventListener('DOMContentLoaded', function () {
    ...
    var canvas_element = $("#sandbox"), canvas_ctx = canvas_element.getContext('2d'), turtle_element = $("#turtle"), turtle_ctx = turtle_element.getContext('2d');
    turtle = new CanvasTurtle(canvas_ctx, turtle_ctx, canvas_element.width, canvas_element.height, $('#overlay'));
    ...
    $('#clearhistory').addEventListener('click', function () {
        if (!confirm(__('Clear history: Are you sure?')))
            return;
        clearhistoryhook();
    });
    $('#clearlibrary').addEventListener('click', function () {
        if (!confirm(__('Clear library: Are you sure?')))
            return;
        logo.run('erall');
    });
