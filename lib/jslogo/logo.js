...
export function LogoInterpreter(turtle, stream, savehook) {
            ...
            lhs = function (lhs) {
                ...
                switch (op) {
                    ...
                    case "/": return defer(function (lhs, rhs) {
                        ...
                        if (d === 0) {
                            throw err("Division by zero", ERRORS.BAD_INPUT);
                        }
                        return n / d;
                    }, lhs, rhs);
                    case "%": return defer(function (lhs, rhs) {
    ...
    self.dispatch = function (name, tokenlist, natural) {
        ...
        if (!procedure) {
            ...
            if ((m = /^(\w+?)(\d+)$/.exec(name)) && self.routines.get(m[1])) {
                throw err("Need a space between {name:U} and {value}", { name: m[1], value: m[2] }, ERRORS.MISSING_SPACE);
            }
        ...
        if (procedure.noeval) {
            return function () {
                self.stack.push(name);
                return promiseFinally(procedure.apply(self, args), function () { self.stack.pop(); });
            };
        }
        return function () {
            self.stack.push(name);
    ...
    function sexpr(atom) {
        if (atom === undefined)
            throw err("Expected string", ERRORS.BAD_INPUT);
        if (atom === UNARY_MINUS)
            return '-';
        if (Type(atom) === 'word')
            return String(atom);
        ...
        return promiseLoop(function (loop, resolve, reject) {
            ...
            Promise.resolve(evaluateExpression(statements))
                ...
                if (result !== undefined && !options.returnResult) {
                    reject(err("Don't know what to do with {result}", { result: result }, ERRORS.BAD_OUTPUT));
                    return;
                }
                lastResult = result;
                loop();
    ...
    function defineProc(name, inputs, optional_inputs, rest, def, block) {
        if (self.routines.has(name) && self.routines.get(name).primitive)
            throw err("{_PROC_}: Can't redefine primitive {name:U}", { name: name }, ERRORS.IS_PRIMITIVE);
        if (def !== undefined &&
            (def < inputs.length || (!rest && def > inputs.length + optional_inputs.length))) {
            throw err("{_PROC_}: Bad default number of inputs for {name:U}", { name: name }, ERRORS.BAD_INPUT);
        }
    ...
    def("def", function (list) {
        ...
        if (!proc.inputs) {
            throw err("{_PROC_}: Can't show definition of primitive {name:U}", { name: name }, ERRORS.IS_PRIMITIVE);
        }
    ...
    def("readlist", function () {
        return ((arguments.length > 0)
            ...
            : stream.read()).then(function (word) {
            return parse('[' + word + ']')[0];
        });
    }, { maximum: 1 });
    ...
    def("filled", function (fillcolor, statements) {
        ...
        turtle.beginpath();
        return promiseFinally(this.execute(statements), function () {
    ...
    def("copydef", function (newname, oldname) {
        ...
        if (this.routines.has(newname)) {
            if (this.routines.get(newname).special) {
                throw err("{_PROC_}: Can't overwrite special {name:U}", { name: newname }, ERRORS.BAD_INPUT);
            }
            if (this.routines.get(newname).primitive && !maybegetvar("redefp")) {
                throw err("{_PROC_}: Can't overwrite primitives unless REDEFP is TRUE", ERRORS.BAD_INPUT);
            }
        }
    ...
    def("repeat", function (count, statements) {
        ...
        var i = 1;
        return promiseFinally(promiseLoop(function (loop, resolve, reject) {
            if (i > count) {
                resolve();
                return;
            }
